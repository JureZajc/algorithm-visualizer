...
import { Alert, Button, inputClassName, Panel, textareaClassName } from "@/components/ui-primitives";
...
export function GraphEditor({
  ...
  return (
    <Panel className="mb-5 p-4 sm:p-5" variant="accent">
      <div className="mb-4">
        <h2 className="m-0 text-lg font-extrabold tracking-normal text-slate-900">Custom graph editor</h2>
        <p className="mb-0 mt-1 text-sm leading-6 text-slate-600">Add nodes on the canvas, then connect and configure them here.</p>
      </div>

      {error ? <Alert className="mb-4" title="Editor input needs attention">{error}</Alert> : null}
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        <div className="rounded-xl border border-slate-200 bg-white p-4">
          ...
          <form className="flex gap-2" onSubmit={beginPlacement}>
            <label className="min-w-0 flex-1 text-xs font-bold text-slate-700">
              Node ID
              <input className={`${inputClassName()} mt-2`} value={nodeId} disabled={disabled || pendingNodeId !== null} placeholder="A" onChange={(event) => setNodeId(event.target.value)} />
            </label>
            <Button className="mt-6" size="sm" variant="primary" type="submit" disabled={disabled || pendingNodeId !== null}>Add node</Button>
          </form>
          {pendingNodeId ? (
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4">
          ...
          <form className="grid grid-cols-2 gap-2" onSubmit={addEdge}>
            <label className="text-xs font-bold text-slate-700">From<select className={`${inputClassName()} mt-2`} value={source} disabled={disabled || nodes.length < 2} onChange={(event) => setEdgeSource(event.target.value)}>{nodes.map((node) => <option key={node.id} value={node.id}>{node.id}</option>)}</select></label>
            <label className="text-xs font-bold text-slate-700">To<select className={`${inputClassName()} mt-2`} value={edgeTargetValue} disabled={disabled || nodes.length < 2} onChange={(event) => setEdgeTarget(event.target.value)}>{nodes.filter((node) => node.id !== source).map((node) => <option key={node.id} value={node.id}>{node.id}</option>)}</select></label>
            <label className="text-xs font-bold text-slate-700">Weight<input className={`${inputClassName()} mt-2`} type="number" step="any" value={edgeWeight} disabled={disabled || nodes.length < 2} onChange={(event) => setEdgeWeight(event.target.value)} /></label>
            <Button className="mt-6" size="sm" variant="soft" type="submit" disabled={disabled || nodes.length < 2}>Add edge</Button>
          </form>
          <div className="mt-3 grid max-h-44 gap-2 overflow-auto">
            {edges.length === 0 ? <span className="text-sm text-slate-500">No edges yet.</span> : edges.map((edge, index) => (
              <div className="grid grid-cols-[minmax(0,1fr)_90px_36px] items-center gap-2 rounded-lg bg-slate-50 p-2 text-sm" key={`${edge.source}-${edge.target}-${edge.weight}-${index}`}>
                ...
                <input
                  className={inputClassName(false, "h-9 px-2 text-sm")}
                  type="number"
                  step="any"
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1">
            <label className="text-xs font-bold text-slate-700">Graph type<select className={`${inputClassName()} mt-2`} value={directed ? "directed" : "undirected"} disabled={disabled || directionLockMessage !== null} onChange={(event) => onDirectedChange(event.target.value === "directed")}><option value="undirected">Undirected</option><option value="directed">Directed</option></select></label>
            ...
            <label className="text-xs font-bold text-slate-700">Start node<select className={`${inputClassName()} mt-2`} value={start} disabled={disabled || nodes.length === 0} onChange={(event) => onStartChange(event.target.value)}>{nodes.map((node) => <option key={node.id} value={node.id}>{node.id}</option>)}</select></label>
            <label className="text-xs font-bold text-slate-700">Target node<select className={`${inputClassName()} mt-2`} value={target} disabled={disabled || nodes.length === 0} onChange={(event) => onTargetChange(event.target.value)}>{nodes.map((node) => <option key={node.id} value={node.id}>{node.id}</option>)}</select></label>
          </div>
        </div>
      </div>
      ...
      <div className="mt-4 rounded-xl border border-slate-200 bg-white p-4">
        <div className="mb-3 flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
          ...
          <div className="flex flex-wrap gap-2">
            <Button size="sm" disabled={disabled} onClick={onExportJson}>Export JSON</Button>
            <Button size="sm" variant="soft" disabled={disabled} onClick={copyJson}>Copy JSON</Button>
          </div>
        </div>
        <form className="grid gap-3" onSubmit={loadJson}>
          <label className="text-xs font-bold text-slate-700">
            Import JSON
            <textarea
              className={`${textareaClassName()} mt-2 min-h-36`}
              value={jsonInput}
              disabled={disabled}
              placeholder={'{\n  "nodes": [\n    { "id": "A", "x": 120, "y": 160 }\n  ],\n  "edges": [],\n  "directed": false,\n  "start": "A",\n  "target": "A"\n}'}
              spellCheck={false}
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            <Button size="sm" variant="primary" type="submit" disabled={disabled}>Load JSON</Button>
